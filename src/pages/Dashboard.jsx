...
export default function Dashboard() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#F4F6F8] md:bg-gradient-to-br md:from-slate-50 md:via-blue-50 md:to-cyan-50 w-full max-w-full relative">
      ...
      <header className="hidden md:block sticky z-40 border-b border-[#E6EAEE] bg-white" style={{ top: 'var(--app-sticky-top, 0px)' }}>
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            />
            <span className="text-lg font-black text-ecg-midnight">PlayECG</span>
          </div>

          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1.5 rounded-full bg-[#FFF1E6] px-3 py-1.5" aria-label={`${streakDays} dias de ofensiva`}>
              <Flame className="h-[17px] w-[17px] text-[#F97316]" />
              <span className="text-base font-black text-[#C2410C]">{streakDays}</span>
            </span>
            <span className="flex items-center gap-1.5 rounded-full bg-[#EEF6FF] px-3 py-1.5" aria-label={`${user?.points || 0} XP`}>
              <Zap className="h-[17px] w-[17px] text-ecg-midnight-2" />
              <span className="text-base font-black text-ecg-midnight-2">{user?.points || 0}</span>
            </span>
            {!isPremium && (
              <Link
                to={createPageUrl("Upgrade")}
                className="rounded-[11px] bg-ecg-green px-3.5 py-2 text-[13px] font-black text-ecg-midnight shadow-[0_3px_0_#16a34a] transition-transform active:translate-y-[2px] active:shadow-[0_1px_0_#16a34a]"
              >
                ASSINAR
              </Link>
            )}
      ...
      <div className="hidden md:block max-w-5xl mx-auto px-4 py-6">
        ...
        <div>
        {/* Quick Actions — o card "Caso do Dia" saiu daqui também. Mesmos
            dois destinos de antes, agora no padrão do redesenho. */}
        <div className="mb-6 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
          <Atalho
            Icone={Shuffle}
            tom="azul"
            titulo="Quiz aleatório"
            legenda="Casos variados, sem ordem"
